...
do
    ...
    if [ "$element" == "Silky by DiMo" ]; then
        ...
        sed -i 's/Exo-2-Bold-Italic_Universal.ttf/NanumSquareNeo-bRg.ttf/g' ../dist/Themes/$element/config.json
        sed -i 's/wqy-microhei.ttc/NanumSquareNeo-cBd.ttf/g' ../dist/Themes/$element/config.json
    else
